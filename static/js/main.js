...
function renderResults(data) {
  ...
  // Counts from 0 to a target number over a given duration.
  // Called when displaying the score so it animates in.
  ...
  function animateCounter(element, target, duration = 600) {
    ...
    function step(timestamp) {
      ...
      // Ease-out: fast at start, slows down at end
  ...
  // ── Score circle ────────────────────────────────────────
  animateCounter(scoreNumber, data.score);;
...
document.getElementById('copy-btn').addEventListener('click', () => {
  // Build a plain-text summary from the current results
  const score = scoreNumber.textContent;
  const strength = scoreStrength.textContent;
  const stats = scoreStats.innerText.replace(/\n/g, ' | ');
  const tips = [...suggestionList.querySelectorAll('li')]
